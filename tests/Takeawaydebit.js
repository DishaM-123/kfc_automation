...
test('KFC order flow', async ({ page }) => {
  ...
  await page.getByRole(locators.cardNumber.role, { name: locators.cardNumber.name }).fill(testData.Debitcard.number);
  await page.getByRole(locators.cardHolder.role, { name: locators.cardHolder.name }).fill(testData.Debitcard.holder);
  await page.getByRole(locators.expiryMonth.role, { name: locators.expiryMonth.name }).fill(testData.Debitcard.expiryMonth);
  await page.getByRole(locators.expiryYear.role, { name: locators.expiryYear.name }).fill(testData.Debitcard.expiryYear);
  await page.getByRole(locators.cvv.role, { name: locators.cvv.name }).fill(testData.Debitcard.cvv);
