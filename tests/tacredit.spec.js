...
test('test', async ({ page }) => {
  ...
  //await page.goto('https://mercury-uat.phonepe.com/transact/simulator?token=3vHK4F49D5Rg1lCZSBOdhhgoq2GvyHlSolvKZxN9Y7ifgrNx6EPdKYTan');
  await page.getByRole('radio', { name: 'Card' }).check();
  await page.getByRole('textbox', { name: 'Enter your card number' }).click();
  await page.getByRole('textbox', { name: 'Enter your card number' }).fill('4242-4242-4242-4242');
  await page.getByRole('textbox', { name: 'Enter card holder’s full name' }).press('CapsLock');
  await page.getByRole('textbox', { name: 'Enter card holder’s full name' }).fill('D');
  await page.getByRole('textbox', { name: 'Enter card holder’s full name' }).press('CapsLock');
  await page.getByRole('textbox', { name: 'Enter card holder’s full name' }).fill('Debit ');
  await page.getByRole('textbox', { name: 'Enter card holder’s full name' }).press('CapsLock');
  await page.getByRole('textbox', { name: 'Enter card holder’s full name' }).fill('Debit C');
  await page.getByRole('textbox', { name: 'Enter card holder’s full name' }).press('CapsLock');
  await page.getByRole('textbox', { name: 'Enter card holder’s full name' }).fill('Debit Card');
  await page.getByRole('textbox', { name: 'MM' }).click();
  await page.getByRole('textbox', { name: 'MM' }).fill('12');
  await page.getByRole('textbox', { name: 'YY' }).fill('25');
  await page.getByRole('textbox', { name: 'CVV' }).fill('936');
